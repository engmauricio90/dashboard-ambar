...
from social_automation.instagram import InstagramAPIError, criar_container_imagem, montar_caption, resumir_image_url, url_midia_temporaria
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if external_url:
            self._testar('URL_EXTERNA_SEM_CAPTION', external_url, '')
            self._testar('URL_EXTERNA_COM_CAPTION', external_url, montar_caption(content))
        else:
            self.stdout.write('URL_EXTERNA: nao executada')

        self.stdout.write('media_publish: NAO executado')

    def _testar(self, label, image_url, caption):
