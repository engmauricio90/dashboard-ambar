...
def lista_planilhas_empreiteiros(request):
    qs = _orcamentos_empresa(request.empresa).filter(tipo=OrcamentoMedicao.TIPO_EMPREITEIRO).select_related('obra')
    obras = _obras_empresa(request.empresa).filter(
        orcamentos_medicao__tipo=OrcamentoMedicao.TIPO_EMPREITEIRO,
    ).distinct().order_by('nome_obra')
    q = request.GET.get('q', '').strip()
    obra_id = request.GET.get('obra', '').strip()
    saldo = request.GET.get('saldo', '').strip()
    ...
    if q:
        qs = qs.filter(Q(nome__icontains=q) | Q(obra__nome_obra__icontains=q) | Q(observacoes__icontains=q))
    obra_filtrada = None
    if obra_id:
        obra_filtrada = obras.filter(id=obra_id).first()
        if obra_filtrada:
    ...
    qs = anotar_resumo_planilhas_contratados(qs)
    if saldo == 'com':
        qs = qs.filter(saldo_otimizado__gt=0)
    elif saldo == 'sem':
        qs = qs.filter(saldo_otimizado__lte=0)
    ...
    query_params.pop('page', None)
    querystring = query_params.urlencode()
    total_empresa = _orcamentos_empresa(request.empresa).filter(tipo=OrcamentoMedicao.TIPO_EMPREITEIRO).count()
    return render(
        request,
        'medicoes/lista_planilhas_empreiteiros.html',
        {
            'page_obj': page_obj,
            'planilhas': page_obj.object_list,
            'obras': obras,
